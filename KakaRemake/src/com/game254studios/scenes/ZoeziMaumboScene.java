...
import com.game254studios.models.ZoeziMaumboModel;
...
public class ZoeziMaumboScene extends ParentScene implements OnClickListener {

	private ZoeziMaumboModel model;
	private BaseGameActivity activity;
	private MyTextureManager textureManager;
	private Engine engine;
	private Camera mCamera;
	private Sprite greenTick,redx;
	public ZoeziMaumboScene(BaseGameActivity act, Engine eng, Camera cam) {
		this.activity = act;
		this.engine = eng;
		this.mCamera = cam;

		model = new ZoeziMaumboModel(activity, engine, mCamera);
	...
	private void setZoeziItems() {
		...
		sprite1.setPosition(-20, -130);
		sprite2.setPosition(-20, mCamera.getHeight() - sprite2.getHeight()+150);
		sprite3.setPosition(mCamera.getWidth() - sprite3.getWidth() + 80, -130);
		sprite4.setPosition(mCamera.getWidth() - sprite4.getWidth() + 80,
				mCamera.getHeight() - sprite4.getHeight() +150);
		...
		if (recent.size() != qaList.size()) {
			...
			new Thread(new Runnable() {
				...
				private void fadeOutandDetachSprites() {
					...
					/*
					 * FadeOutModifier prFadeOutModifier = new
					 * FadeOutModifier(1.0f,EaseLinear.getInstance());
					 * sprite1.setBlendFunction(GL10.GL_SRC_ALPHA,
